...
res.pca=PCA(exam,scale.unit = TRUE,ncp=4,graph = TRUE)
...
k5.result=kmeans(exam,5,nstart = 25)
k4.result=kmeans(exam,4,nstart = 25)
k3.result=kmeans(exam,3,nstart = 25)
k2.result=kmeans(exam,2,nstart = 25)

p1=fviz_cluster(k2.result, geom = "point",data = exam)+ ggtitle("k = 2")
p2=fviz_cluster(k3.result, geom = "point",data = exam)+ ggtitle("k = 3")
p3=fviz_cluster(k4.result, geom = "point",data = exam)+ ggtitle("k = 4")
p4=fviz_cluster(k5.result, geom = "point",data = exam)+ ggtitle("k = 5")

grid.arrange(p1, p2, p3, p4, nrow = 2)

set.seed(123)
fviz_nbclust(exam, kmeans, method = "wss")

cluster.exam=HCPC(res.pca)
head(cluster.exam$data.clust)
cluster.exam$desc.var
cluster.exam$desc.axes
cluster.exam$desc.ind